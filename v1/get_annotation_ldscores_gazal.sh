...
rm $f"_500bp_ext"
cp $path_name$all_annotations/*annot.gz $f"_gazal_dir"
cd $f"_gazal_dir"
gunzip *
for j in *.annot
do
echo $j
perl $path_name/fast_match2.pl ../$f".1000genomes.intersect.snp" $f $j > a
perl $path_name/fast_match2.pl ../$f"_500bp_ext.1000genomes.intersect.snp" $f"_500bp_ext" a > $j".txt"
done
gzip *annot.txt
rm *.annot
rm a
