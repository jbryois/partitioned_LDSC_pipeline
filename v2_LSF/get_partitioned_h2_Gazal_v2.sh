...
echo $gwas_name
cd $f
bsub -o log_gazal_$gwas_name "ldsc.py --h2 $sumstats --ref-ld-chr $path_name$all_annotations/baselineLD.,baseline. --w-ld-chr $path_name$weights --overlap-annot --frqfile-chr $path_name$frq --print-coefficients --out ../${gwas_name}_${f}"
cd ..
done